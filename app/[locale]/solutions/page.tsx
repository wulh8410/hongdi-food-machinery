...
export async function generateMetadata({ params }: { params: { locale: Locale } }): Promise<Metadata> {
  ...
  return localizedMetadata({ locale: params.locale, path: `/${params.locale}/solutions`, title: `${site.nav.solutions} | ${site.name}`, description: params.locale === 'zh' ? '中小型屠宰档口、养殖场、食堂餐饮、水产门店和食品加工设备解决方案。' : 'Solutions for slaughter stalls, farms, canteens, restaurants, aquatic stores, and food processing equipment.', keywords: site.keywords });
}
...
export default async function SolutionsPage({ params }: { params: { locale: Locale } }) {
  ...
  const categoryOrder = params.locale === 'zh' ? ['家禽加工', '水产加工', '场地与配套'] : [];
