...
export default async function FAQDetailPage({ params }: { params: { locale: Locale; slug: string } }) {
  ...
  const [products, articles, solutions] = await Promise.all([
    ...
    getContentItems(params.locale, 'solutions')
  ]);
  ...
  return (
    <PageShell locale={params.locale} path={`/${params.locale}/faqs/${params.slug}`}>
      ...
      <article className="mx-auto max-w-4xl px-4 py-8 md:px-6">
        ...
        <Section title={params.locale === 'zh' ? '答案' : 'Answer'}>
          <p className="leading-8 text-slate-700">{a}</p>
        </Section>
        <Section title={params.locale === 'zh' ? '采购说明' : 'Buying Notes'}>
          <div className="content" dangerouslySetInnerHTML={{ __html: item.bodyHtml }} />
        </Section>
