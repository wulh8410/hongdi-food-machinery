...
当前内容已从“示例内容骨架”升级为“洪弟食品机械双语采购知识库”。内容围绕真实采购问题、设备选型、使用场景、安装维护、故障判断和厂家咨询路径组织，并保持中英文页面结构和数量一致。
...
当前产品图、详情图和参数资料已统一整理到 `docs/洪弟食品机械产品列表资料.md`，官网产品中心按同类设备排序。旧占位产品、水产设备和鱼糜丸类内容不再纳入当前产品清单。
...
## 26. 2026-06-27 第四批内容文章继续补充记录

### 发布目标

- 在已有第四批文章基础上继续补充安装、清洗、排水、收工检查和多设备协同类内容。
- 继续面向真实客户问题写作，让客户看完能判断现场怎么准备、设备怎么摆放、使用中怎么清洗维护。
- 不在客户可见正文中使用内部 GEO、AI、搜索引擎等运营话术。

### 本轮新增中文文章

1. 家禽脱毛设备安装时，为什么要预留检修和清洗通道？
2. 家禽脱毛机出料口和接料台怎么配，才能减少搬运？
3. 泡水机和脱毛机排水口怎么布置，现场更容易清洗？
4. 清洗家禽脱毛机时，能不能直接用高压水枪冲？
5. 档口收工后，家禽脱毛设备要检查哪些地方？
6. 设备送到养殖场或乡镇加工点，收货前要确认什么？
7. 两台以上家禽脱毛机同时使用，现场怎么安排更顺？
8. 选食品机械不锈钢设备时，边角和清洗死角要怎么看？
9. 鸡鸭鹅混合处理时，为什么要分批次安排？
10. 食堂和中央厨房采购脱毛设备，怎么估算高峰处理量？

### 内容口径

- 每篇文章都保留洪弟食品机械品牌露出、相关设备、关联 FAQ 和联系方式。
- 重点围绕客户现场会遇到的具体问题，不写泛泛行业科普。
- 联系方式继续统一为“电话：13729374860（微信同号）”和工厂地址。

## 27. 2026-08-25 第三轮迭代升级前基线

### 当前线上入口

- 自定义域名：`https://hd.hong1234.com/zh/`
- Vercel：`https://hongdi-food-machinery.vercel.app/zh/`
- GitHub Pages：`https://wulh8410.github.io/hongdi-food-machinery/zh/`
- 2026-08-25 检查结果：三个入口均返回 HTTP 200。

### 当前版本与内容规模

- 第三轮升级前源码基线：`main` 分支，上一版提交为 `d13c9c5`。
- 产品：中文 11 个、英文 11 个。
- 文章：中文 306 篇、英文 306 篇。
- FAQ：中文 451 个、英文 451 个。
- 解决方案：中文 148 个、英文 148 个。
- 当前产品列表资料：`docs/洪弟食品机械产品列表资料.md`。

### 页面效果图基线

- 基线目录：`docs/qa/third-round-baseline-20260825/`。
- 统一使用 1440×900 桌面视口，保存全页 JPG 截图。
- 中文 11 张、英文 11 张，共 22 张。
- 覆盖首页、产品列表、产品详情、解决方案列表、解决方案详情、FAQ 列表、FAQ 详情、文章列表、文章详情、关于我们和联系我们。
- 截图文件、路由和用途见该目录内 `README.md`。

### 本次备份边界

- 纳入：官网源码、已确认的项目文档、当前产品列表资料、第三轮升级前页面截图。
- 不纳入：本机 `produce/`、`img2/`、历史图片备份、画册草稿和大体积抖音抓取原始文件；这些属于源素材或研究数据，不应直接扩大官网代码仓库。
- Git 提交备注统一使用：`第三轮迭代升级前备份`。

### 第三轮升级原则

- 先以本次截图为视觉对照，再调整组件和页面，避免无法判断改版是否真正改善整体感。
- 保持现有双语路由、静态导出、GEO 文件、结构化数据和内容数据结构不被视觉改版破坏。
- 新版验收至少复查本次覆盖的 11 种页面模板，并同时检查中文、英文和移动端。
