...
## 23. 2026-06-21 首页第二版视觉升级记录

### 升级背景

- 现有首页整体视觉仍偏“模块拼接”，工业感、专业感和整体连续性不足。
- 用户确认使用 `claude-design` 方向进行首页第二版升级，要求先以效果图确认，再按确认方向落地代码。
- 升级前已创建基线提交：`01b162e 网站进行第二版升级`。

### 第二版首页设计原则

- 视觉方向：工业风、大气、专业、B2B 食品机械工厂官网。
- 视觉系统：深海军蓝作为主视觉背景，钢灰浅色区承载阅读内容，橙色仅作为品牌识别和关键行动点。
- 素材策略：优先使用真实门头图、横版 Logo 和前 4 款主推产品图，不使用虚构插画或装饰性假图。
- 结构策略：按照采购判断路径组织首页，即“门头主视觉 -> 工厂定位 -> 主推设备 -> 场景解决方案 -> GEO 内容入口 -> 底部联系信息”。
- 信息密度：保持 B2B 官网应有的信息密度，但减少零散卡片感，统一边框、色块、标题和按钮语言。

### 本次代码改版范围

- `components/Header.tsx`
  - 改为白底横版 Logo + 紧凑导航结构。
  - Logo 使用 `hongdi-logo-header.png`，与新版品牌素材保持一致。

- `components/HeroSection.tsx`
  - 改为全宽门头主视觉。
  - 使用 `hongdi-factory-equipment-banner.png` 作为主视觉背景。
  - 增加设备系统信息面板、核心能力标签和四项工厂数据。

- `components/Cards.tsx`
  - 优化产品卡片比例、图片展示、标签和内容排版。
  - 适配首页四列主推设备矩阵。

- `app/[locale]/page.tsx`
  - 重构首页主体。
  - 保留 4 个主推产品、4 个解决方案、4 个热门 FAQ、4 篇最新文章。
  - 移除首页额外联系 CTA，避免与底部联系方式重复。

- `components/Footer.tsx`
  - 重构底部为品牌信息、快速入口、主营设备三栏。
  - 联系方式改为横排信息块，减少左侧内容过长、右侧过空的问题。

### 验收重点

- 首页整体必须比上一版更统一，减少深浅区块之间的割裂感。
- Logo、头图、产品图必须正常加载。
- 主推产品保持一行 4 个。
- 解决方案、FAQ、文章入口都保持 4 个内容展示。
- 移动端仍需保持可读和可访问。
- 构建验证需执行 `npm run build`。
